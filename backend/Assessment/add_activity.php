...
try {
    ...
    $duplicate_check = $conn->prepare('
        ...
        WHERE a.advisory_id = ? AND a.subject_id = ? AND a.activity_date = ? AND s.stud_schedule_class = ?
    ');
    ...
    $stmt = $conn->prepare('SELECT COALESCE(MAX(activity_num), 0) + 1 AS next_activity FROM tbl_activities WHERE advisory_id = ? AND subject_id = ? AND quarter_id = ?');
    ...
    $insertActivity = $conn->prepare('INSERT INTO tbl_activities (advisory_id, subject_id, quarter_id, activity_name, activity_date, activity_num) VALUES (?, ?, ?, ?, ?, ?)');
    ...
    $stmt = $conn->prepare('SELECT activity_id FROM tbl_activities WHERE advisory_id = ? AND subject_id = ? AND quarter_id = ? ORDER BY activity_date ASC, activity_id ASC');
    $stmt->execute([$advisory_id, $subject_id, $quarter_id]);
