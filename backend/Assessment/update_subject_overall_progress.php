...
// 2. Check if subject overall progress records already exist
...
if (!$checkRow || intval($checkRow['cnt']) == 0) {
    echo json_encode(["status" => "error", "message" => "No existing subject overall progress records found. Use insert_subject_overall_progress.php instead."]);
    exit;
}
...
logShapeMapping("update_subject_overall_progress", $shapeAvg, $conn);
...
// 7. Update existing records (no insert/delete)
$updatedRows = [];
$updateStmt = $conn->prepare('
    UPDATE tbl_subject_overall_progress 
    SET finalsubj_visual_feedback_id = ?, finalsubj_avg_score = ?, computed_at = NOW() 
    WHERE student_id = ? AND subject_id = ? AND advisory_id = ?
');
...
if (empty($updatedRows)) {
    echo json_encode(["status" => "error", "message" => "No subjects with complete feedback for all quarters to update."]);
    exit;
}

echo json_encode(["status" => "success", "updated" => $updatedRows, "message" => "Subject overall progress records updated successfully."]); 