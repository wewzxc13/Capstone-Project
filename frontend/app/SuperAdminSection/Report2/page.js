...
import { FaBell, FaCog, FaChevronDown, FaEdit, FaMale, FaFemale, FaUsers } from "react-icons/fa";
  ...
  const renderRightCard = () => {
    switch (selectedReport) {
         ...
         if (loading) {
           return (
             <>
               <h3 className="text-sm sm:text-lg font-semibold text-[#232c67] mb-2">
                 Total of Student
               </h3>
                ...
                <table className="text-xs sm:text-sm text-gray-700 w-full">
                  <thead className="border-b">
                   ...
                   {attendanceData.levelNames.map((levelName, index) => (
                     <th key={levelName} className={`text-center ${colors[index]}`}>
                       {levelName}
                     </th>
                   ))}
                    </tr>
         ...
         return (
           <>
             ...
             <table className="text-xs sm:text-sm text-gray-700 w-full">
               <thead className="border-b">
                 <tr>
                   ...
                   {progressData.levelNames.map((levelName, index) => (
                     <th key={levelName} className={`text-center ${progressColors[index]}`}>
                       {levelName}
                     </th>
                   ))}
                 </tr>
               ...
               <tbody>
                 <tr>
                   <td className="py-1 text-green-600 flex items-center gap-2">
                 ...
                 <tr>
                   <td className="py-1 text-yellow-600 flex items-center gap-2">
                   ...
                   {progressData.levelNames.map(levelName => (
                     <td key={`moderate-${levelName}`} className="text-center">
                       {progressData.riskLevelData[levelName]?.['Moderate'] || 0}
                                   ...
                                   <tr>
                    <td className="py-1 text-red-600 flex items-center gap-2">
               ...
               <thead className="border-b">
                 <tr>
                   ...
                   {subjectData.levelNames.map((levelName, index) => (
                     <th key={levelName} className={`text-center ${subjectColors[index]}`}>
                       {levelName}
                     </th>
                   ))}
                 </tr>
    ...
    <ProtectedRoute role="Super Admin">
      <div className="flex flex-col md:flex-row bg-[#f0f6ff] min-h-screen">
        <main className="flex-1 p-2 sm:p-4 md:p-6">
          {/* Header Section with Navy Blue Theme */}
          <div className="bg-[#232c67] p-6 rounded-t-xl text-white mb-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
              ...
              <div className="relative mt-4 sm:mt-0">
                <button
                  ...
                  className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg px-4 py-2 text-sm font-medium text-white hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/50 transition-all duration-200"
                >
                  View Report
                    ...
                    {reports.map((report) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 ${
                          selectedReport === report ? 'bg-[#232c67] text-white' : 'text-gray-700'
                        }`}
                      >
                        {report}
                      </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 col-span-1 lg:col-span-2">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-2 h-8 bg-[#232c67] rounded-full"></div>
                <h3 className="text-lg font-semibold text-[#232c67]">
                  Insight Summary
                </h3>
              </div>
              <div className="h-40 overflow-y-auto border border-gray-100 rounded-lg p-4 bg-gray-50/50">
            ...
            <div className="relative" style={{ height: '320px' }}>
              ...
              {selectedReport === "Attendance Report" && attendanceData && !attendanceData.hasData && (
                <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-95 rounded-lg">
                  <div className="text-center text-gray-500 text-sm p-6">
                    ...
                    <div className="font-medium mb-1">No attendance data available</div>
                    <div className="text-xs text-gray-400">Data will appear once teachers start recording attendance</div>
                  </div>
                </div>
              )}
              {selectedReport === "Progress Report" && progressData && !progressData.hasData && (
                <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-95 rounded-lg">
                  <div className="text-center text-gray-500 text-sm p-6">
                    <div className="mb-3 text-4xl">📊</div>
                    <div className="font-medium mb-1">No progress data available</div>
                    <div className="text-xs text-gray-400">Data will appear once progress assessments are recorded</div>
                  </div>
                </div>
              )}
              {selectedReport === "Subject Report" && subjectData && !subjectData.hasData && (
                <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-95 rounded-lg">
                  <div className="text-center text-gray-500 text-sm p-6">
                    <div className="mb-3 text-4xl">📊</div>
                    <div className="font-medium mb-1">No subject data available</div>
                    <div className="text-xs text-gray-400">Data will appear once subject assessments are recorded</div>
                  </div>
                </div>
              )}
            </div>
