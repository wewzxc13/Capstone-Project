...
export default function StudentAssessment({ student, onBack, onRiskUpdate, triggerExport, onExportComplete, parentProfile, studentLevelData, advisory }) {
  ...
  // Download PDF: use shared jsPDF generator so both tabs match exactly
    ...
    const fetchAllData = async () => {
      try {
        ...
        if (data[0].status === 'success') {
          ...
          console.log('🎯 Visual Feedback loaded:', data[0].feedback);
        } else {
          console.log('❌ Visual Feedback failed:', data[0]);
        }
