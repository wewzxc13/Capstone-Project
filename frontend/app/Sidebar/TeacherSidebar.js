    ...
    const fetchCounts = async () => {
      try {
        ...
        const usersUnread = Array.isArray(recentRes?.data)
          ...
          : 0;
        // Apply the same visibility rules as the Teacher Messages page
        const teacherFullName = userRes && userRes.status === 'success' && userRes.user && userRes.user.fullName ? String(userRes.user.fullName) : '';
        const advisoryLevelName = advRes && advRes.advisory && advRes.advisory.level_name ? String(advRes.advisory.level_name) : '';
        const teacherLower = teacherFullName.trim().toLowerCase();
        const groups = Array.isArray(groupsRes?.data) ? groupsRes.data : [];
        const eligibleGroups = groups.filter((g) => {
          const type = String(g.group_type || '').toLowerCase();
          if (type !== 'class') return true; // allow Overall/Staff
          // Only count class groups if teacher is adviser or level matches
          const label = String(g.participants_label || '').toLowerCase();
          if (teacherLower && label.includes(teacherLower)) return true;
          if (advisoryLevelName) {
            const lvl = advisoryLevelName.toLowerCase();
            if (String(g.group_name || '').toLowerCase().includes(lvl)) return true;
          }
          return false;
        });
  ...
  const SidebarContent = ({ isSidebarOpen, onNavClick }) => (
    ...
    >
      ...
      <div
        ...
        onClick={() => (window.location.href = "/TeacherSection/Dashboard")}
