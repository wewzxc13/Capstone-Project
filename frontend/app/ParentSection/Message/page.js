...
export default function ParentMessagesPage() {
  ...
  const pollForNewMessages = async () => {
    ...
    try {
      ...
      if (activeTab === 'Groups') {
        ...
        const recentGroupData = await recentGroupRes.json();
    ...
    fetch(`${API.communication.getRecentConversations()}?user_id=${uid}`, {
      ...
      .then(async (json) => {
        ...
        const mapped = (json.data || []).map((u) => {
          ...
          // Remove any existing "You: " prefix to prevent duplication
          if (fromSelf && rawLast.startsWith('You: ')) {
            rawLast = rawLast.substring(5);
          }
  ...
  function getQuickReplies(type, chat) {
    ...
    if (!chat) return [
      ...
      "Can you update me on activities?",
    ...
    if (type === 'group') {
      ...
      if (gt === 'staff') {
        return [
          ...
          "Can you share the class schedule?",
      ...
      if (gt === 'class') {
        return [
          "When are the upcoming field trips?",
          "What activities is due this week?",
          "Can you share the class schedule?",
      ...
      return [
        ...
        "Can you share the newsletter?",
        "What's happening in the school this week?",
    ...
    switch (Number(chat.role)) {
        case 4: // Parent to Parent
          return [
            "How's school going?",
            "What activities this week?",
            "Did you hear about the meeting?",
            "Any parenting tips?",
            "How's class time going?",
          ];
        case 3: // Parent to Teacher
          return [
            "How is my child doing?",
        ...
        case 2: // Parent to Admin
          return [
            "What's the enrollment process?",
            "When is the next meeting?",
            "I have policy concerns.",
            "Any calendar updates?",
            "I'd like to give feedback.",
          ];
        case 1: // Parent to Owner
          return [
            "I'm concerned about direction.",
            "Can you explain policies?",
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => router.push('/ParentSection/Dashboard')}
