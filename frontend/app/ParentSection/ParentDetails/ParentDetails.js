...
function getInputClassName(fieldName, formData, validationErrors) {
  const baseClass = "border w-full p-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-[#232c67] focus:border-[#232c67] cursor-text caret-[#1E2A79]";
...
export default function ParentDetails() {
  ...
  // Note: Toast checking removed - now handled in Dashboard
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (data.status === "success") {
          const userData = {
            ...
            user_photo: data.user.photo || data.user.user_photo || ""
          };
        ...
        toast.error("Error loading user profile. Please check your connection and try again.");
      } finally {
        setIsLoading(false);
      }
    };
    ...
    // PSGC address data setup (same as SuperAdminSection/ViewOwnUser)
  ...
  const handleSave = async () => {
    try {
        ...
        if (cleanDigits.length === 10 && cleanDigits.startsWith('9')) {
          ...
          if (cleanDigits.length >= 10) {
            ...
            dataToSend.user_contact_no = '';
              ...
              // Prepare payload for parent: map city to municipality_city
        const payload = {
          user_id: userId,
          ...dataToSend,
      ...
      if (data.status === "success") {
        ...
        // Update the user name in the context so Topbar reflects the change immediately
        const newFullName = [dataToSend.user_firstname, dataToSend.user_middlename, dataToSend.user_lastname]
          .filter(name => name && name.trim())
          .join(' ')
          .trim();
        
        console.log('Updating user name in context:', { 
          oldName: formData.user_firstname + ' ' + formData.user_middlename + ' ' + formData.user_lastname,
          newName: newFullName,
          individualNames: {
            first: dataToSend.user_firstname,
            middle: dataToSend.user_middlename,
            last: dataToSend.user_lastname
          }
        });
        
        updateUserName(newFullName);
        
        // Update local formData to reflect changes immediately in the UI
        setFormData(prevData => ({
          ...prevData,
          user_firstname: dataToSend.user_firstname,
          user_middlename: dataToSend.user_middlename,
          user_lastname: dataToSend.user_lastname
        }));
  ...
  const openCropModal = (imageFile) => {
    ...
    if (imageFile instanceof File && imageFile.size > 0) {
      ...
      const currentPhotoUrl = getPhotoUrl(formData.user_photo);
      if (currentPhotoUrl) {
  ...
  const createCroppedFile = (blob) => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        user_photo: previewUrl // This is just for preview, not for database storage
      }));
  ...
  function validateForm(newFormData = formData) {
    ...
    [
      ...
      'user_birthdate', 'barangay', 'city', 'province', 'country'
    ].forEach(field => {
  ...
  useEffect(() => {
    return () => {
      ...
      if (formData.user_photo && formData.user_photo.startsWith('blob:')) {
        try {
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between w-full">
                <div>
                  <div className="text-xl font-bold text-gray-900">
                    {formData.user_lastname ? `${formData.user_lastname}, ${formData.user_firstname} ${formData.user_middlename || ''}`.trim() : `${formData.user_firstname} ${formData.user_middlename} ${formData.user_lastname}`.trim()}
                  </div>
              ...
              <div>
                ...
                {isEditing ? (
                  <input
                    ...
                    onChange={handleChange}
                    className={`w-full rounded-lg p-2 ${getInputClassName('user_contact_no', formData, validationErrors)} caret-[#1E2A79]`}
              ...
              <div>
                ...
                {isEditing ? (
                  <select
                    ...
                    onChange={handleChange}
                    className={`${getInputClassName('country', formData, validationErrors)} cursor-pointer`}
                  >
              ...
              <div>
                ...
                {isEditing ? (
                  <select
                    ...
                    onChange={handleChange}
                    className={`${getInputClassName('province', formData, validationErrors)} cursor-pointer`}
              ...
              <div>
                ...
                {isEditing ? (
                  <select
                    ...
                    value={formData.city || ""}
                    onChange={handleChange}
                    className={`${getInputClassName('city', formData, validationErrors)} cursor-pointer`}
              ...
              <div>
                ...
                {isEditing ? (
                  <input
                    ...
                    onChange={handleChange}
                    className={`${getInputClassName('barangay', formData, validationErrors)} caret-[#1E2A79]`}
                    ...
                    <div className="text-gray-600">
                     ...
                     <div className="text-gray-500">
                       ...
                       <div className={`text-xs ${isEditing ? 'text-blue-600' : 'text-gray-400'}`}>
                         {isEditing ? 'Click photo to upload your first photo' : 'No photo'}
                       </div>
                     </div>
                   )}
                </div>
