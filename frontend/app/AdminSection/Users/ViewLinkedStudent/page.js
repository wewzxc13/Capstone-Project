...
export default function ViewLinkedStudentPage() {
  ...
  const handleViewStudent = (student) => {
    console.log('handleViewStudent called with:', student);
            console.log('Navigating to student profile:', `/AdminSection/Users/ViewUser?id=${student.id}&role=Student`);
        router.push(`/AdminSection/Users/ViewUser?id=${student.id}&role=Student`);
  };
    ...
    <ProtectedRoute role="Admin">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 mb-0 flex-shrink-0 z-10 fixed top-0 left-0 sm:left-64 right-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-3">
            ...
            <h2 className="text-base sm:text-lg font-bold text-gray-900">Linked Students Management</h2>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:justify-between mb-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <div className="flex items-center gap-2 px-2 py-1 bg-[#e8ecf7] rounded-full w-fit">
                <FaUsers className="text-[#232c67] text-xs sm:text-sm" />
              ...
              <div className="relative parent-view-dropdown w-full sm:w-auto">
                <button
                  type="button"
                  onClick={() => setParentViewDropdownOpen(!parentViewDropdownOpen)}
                  className="flex items-center justify-between bg-white border border-gray-300 hover:border-gray-400 text-gray-700 px-2 sm:px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors shadow-sm hover:shadow-md w-full sm:min-w-[200px]"
                >
        ...
        {error ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 flex-shrink-0 mt-24 sm:mt-32">
            <div className="flex flex-col justify-center items-center h-32 sm:h-48 text-center">
              <p className="text-red-600 mb-3 sm:mb-4 text-base sm:text-lg font-medium">{error}</p>
              <button 
                onClick={() => window.location.reload()} 
                className="bg-[#232c67] text-white px-4 sm:px-6 py-2 rounded-lg hover:bg-[#1a1f4d] transition-colors font-medium text-sm sm:text-base"
              >
                Retry
              </button>
            </div>
          ...
          <div className="mt-24 sm:mt-32 h-[400px] sm:h-[550px] overflow-y-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 p-1">
            {filteredParentList.length === 0 ? (
              <div className="col-span-full bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 flex flex-col">
                <div className="flex flex-col justify-center items-center text-center">
                  <FaUsers className="text-3xl sm:text-4xl text-gray-300 mb-3 sm:mb-4" />
                  <p className="text-base sm:text-lg font-medium text-gray-500">No parents found</p>
                  <p className="text-xs sm:text-sm text-gray-400 mt-2">Parents will appear here once they are added to the system.</p>
                </div>
              </div>
            ) : (
              <>
                   ...
                   sortedParentsWithActiveStudents.length === 0 ? (
                     <div className="col-span-full bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col">
                      ...
                      return (
                       <div key={parent.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 cursor-pointer hover:shadow-md transition-shadow" onClick={() => router.push(`/AdminSection/Users/ViewUser?id=${parent.id}&role=Parent`)}>
                        {/* Parent Header */}
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:justify-between mb-3 sm:mb-4">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              className="flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 bg-[#232c67] text-white rounded-lg font-semibold hover:bg-[#1a1f4d] transition-colors focus:outline-none focus:ring-2 focus:ring-[#232c67] focus:ring-offset-2 shadow-sm text-xs sm:text-sm w-full sm:w-auto justify-center sm:justify-start"
                            ...
                            >
                              ...
                              <span className="hidden xs:inline">Link Student</span>
                            ...
                            <div className={`${parent.linkedStudents.length > 1 ? 'max-h-24 overflow-y-auto' : ''}`}>
                              <div className="space-y-2">
                                {sortStudentsAlphabetically([...parent.linkedStudents]).map((student, index) => (
                                  <div key={`${student.id}-${student.schoolStatus}-${index}`} className="flex items-center gap-2 sm:gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors border border-gray-100 cursor-pointer" onClick={(e) => { e.stopPropagation(); handleViewStudent(student); }}>
                                    <div 
                                    ...
                                    >
                                      ...
                                      <div className="flex-1 min-w-0">
                                        <div className="font-medium text-gray-900 text-sm sm:text-base truncate">{formatName(student.name)}</div>
                   ...
                   sortedParentsWithOnlyInactiveStudents.length === 0 ? (
                     ...
                     sortedParentsWithOnlyInactiveStudents.map((parent) => {
                       ...
                       return (
                         <div key={parent.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 cursor-pointer hover:shadow-md transition-shadow" onClick={() => router.push(`/AdminSection/Users/ViewUser?id=${parent.id}&role=Parent`)}>
                          {/* Parent Header */}
            ...
            {isModalOpen && (
              <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
                <div className="bg-white rounded-xl shadow-2xl p-4 sm:p-8 w-full max-w-[520px] relative border border-gray-100">
                  <div className="mb-3 sm:mb-4">
                    <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-1">Link Student to Parent</h3>
                  ...
                  <div className="max-h-48 sm:max-h-60 overflow-y-auto border border-gray-200 rounded-lg">
                    {availableStudents.length === 0 ? (
                      <div className="flex flex-col justify-center items-center h-24 sm:h-32 text-center">
                        <FaUsers className="text-3xl sm:text-4xl text-gray-300 mb-2" />
                        <p className="text-gray-500 text-sm sm:text-base">No available students to link</p>
                      </div>
                    ) : (
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-gray-200 mt-4 sm:mt-6">
                    <button 
                      className="flex items-center justify-center gap-2 bg-gray-500 hover:bg-gray-600 text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg font-medium transition-colors w-full sm:w-auto" 
                    ...
                    <button 
                      className={`flex items-center justify-center gap-2 px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg font-medium transition-colors shadow-md w-full sm:w-auto ${
                        !selectedStudent
            ...
            {showRemoveModal && (
              <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
                <div className="bg-white rounded-xl shadow-2xl p-4 sm:p-8 w-full max-w-[520px] relative border border-gray-100">
                  <div className="mb-3 sm:mb-4">
                    <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-1">Remove Linked Student</h3>
                  ...
                  <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="font-medium text-gray-900 text-sm sm:text-base truncate">{formatName(studentToRemove?.name)}</div>
                        <div className="text-xs sm:text-sm text-gray-500">Student will be unlinked from parent</div>
                      </div>
                    </div>
                  </div>
